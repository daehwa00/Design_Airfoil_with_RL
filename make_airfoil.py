...
def interpolate_linear_functions(hull_points, N=100):
    ...
    upper_intercepts = upper_hull_points[:-1, 1] - upper_slopes * upper_hull_points[:-1, 0]

    # N+1을 사용하고 endpoint=False를 추가합니다.
    upper_front_x_values = np.geomspace(0.0001, 0.1, N_front, endpoint=False)  # 0 대신 최소값으로 시작
    ...
    for i in range(N):
        ...
        while upper_current_segment < len(upper_slopes) - 1 and upper_x > upper_hull_points[upper_current_segment + 1, 0]:
            upper_current_segment += 1
        upper_y_values[i] = upper_slopes[upper_current_segment] * upper_x + upper_intercepts[upper_current_segment]
    ...
    lower_intercepts = lower_hull_points[:-1, 1] - lower_slopes * lower_hull_points[:-1, 0]

    lower_front_x_values = np.geomspace(0.0001, 0.1, N_front, endpoint=False)  # 0 대신 최소값으로 시작
    ...
    for i in range(N):
        ...
        while lower_current_segment < len(lower_slopes) - 1 and lower_x > lower_hull_points[lower_current_segment + 1, 0]:
            lower_current_segment += 1
        lower_y_values[i] = lower_slopes[lower_current_segment] * lower_x + lower_intercepts[lower_current_segment]
...
def get_airfoil_points(circles, N=80, plot = False):
    ...
    if plot:
        ...
        plt.plot(np.array(hull_points[:, 0]), np.array(hull_points[:, 1]), 'o-', label='Hull Points')
        plt.plot(np.array(interpolated_points[:, 0]), np.array(interpolated_points[:, 1]), '.r', label='Interpolated Points')
        plt.legend()
        plt.xlabel('X')
        plt.ylabel('Y')
